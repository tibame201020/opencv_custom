...
- **Frontend / Workflow Canvas**: Structural Overhaul (Phase 4) — Removed absolute positioning from Node titles and redefined `N8nNode.tsx` as an elastic width horizontal card layout (`min-w-[160px] max-w-[240px]`), ensuring precise Handle boundaries and completely eliminating instances where connection lines and execution badges (like '1 item') overlapped with node text.
