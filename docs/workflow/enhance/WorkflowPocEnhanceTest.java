...
public class WorkflowPocEnhanceTest {
    ...
    @Test
    @DisplayName("Test 6: Nested Subflow - 驗證多層遞迴執行與信號傳遞")
    void testNestedSubflow() {
        // Level 2: SubB - Simple identity
        Workflow<Object, Object> subB = new Workflow<>();
        subB.addNode(WorkflowNode.createConvertNode("b1", "bInit", i -> "B(" + i + ")"));

        // Level 1: SubA - Calls SubB
        Workflow<Object, Object> subA = new Workflow<>();
        subA.addNode(WorkflowNode.createSubWorkflowNode("a1", "callSubB", subB))
                .addNode(WorkflowNode.createConvertNode("a2", "wrapA", s -> "A[" + s + "]"));
        subA.addEdge(new WorkflowEdge("ae1", "a1", "a2", "success"));

        // Level 0: Parent
        Workflow<Object, Object> parent = new Workflow<>();
        parent.addNode(WorkflowNode.createSubWorkflowNode("p1", "callSubA", subA))
                .addNode(WorkflowNode.createConvertNode("p2", "wrapParent", s -> "P{" + s + "}"));
        parent.addEdge(new WorkflowEdge("pe1", "p1", "p2", "success"));

        ExecutionResult<Object> result = new FlowEngine<>(parent).execute("input");
        assertThat(result.output()).isEqualTo("P{A[B(input)]}");
        assertThat(result.executionPath()).containsExactly("p1", "p2");
    }
}
